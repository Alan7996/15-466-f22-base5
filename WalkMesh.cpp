...
glm::vec3 barycentric_weights(glm::vec3 const &a, glm::vec3 const &b, glm::vec3 const &c, glm::vec3 const &pt) {
	//TODO: implement!
	// compute barycentric coordinate using subtriangle areas
...
void WalkMesh::walk_in_triangle(WalkPoint const &start, glm::vec3 const &step, WalkPoint *end_, float *time_) const {
	...
	glm::vec3 step_coords = barycentric_weights(a, b, c, to_world_point(start) + step) - start.weights;
	
	//figure out which edge (if any) is crossed first.
	// set time and end appropriately.
	//TODO

	// inspired by Sacha Bartholme's work
	...
	time = 1.0f;
	if (tx > 0.0f) time = std::min(time, tx);
	if (ty > 0.0f) time = std::min(time, ty);
	if (tz > 0.0f) time = std::min(time, tz);
	...
	if (std::abs(end.weights.x) < 0.0001f) {
	...
	} else if (std::abs(end.weights.y) < 0.0001f) {
...
bool WalkMesh::cross_edge(WalkPoint const &start, WalkPoint *end_, glm::quat *rotation_) const {
	...
	if (it != next_vertex.end() /* <-- TODO: use a real check, this is just here so code compiles */) {
		//it is!

		//make 'end' represent the same (world) point, but on triangle (edge.y, edge.x, [other point]):
		//TODO
		end.indices = glm::vec3(start.indices.y, start.indices.x, it->second);
